...
export default class FirebaseService {
  ...
  signInWithCredential=async (username, password)=>{
    try {
      let prevUser = this.auth.currentUser;

      const credential = firebaseRN.auth.EmailAuthProvider.credential(username, password);
      const userCredential = await this.auth.signInWithCredential(credential).catch((error) =>{
  ...
  startListenData = async (channel)=>{
    ...
    try {
      ...
      if (channel) {
        ...
        const snapshot =  await this.database.ref(path).once('child_added');
        if (snapshot.exists()) {
        ...
        // });
          ...
          if (!_.isEmpty(data)) {
            ...
            if (action) {
              ...
              result  && !_.isEmpty(fuid)&& !_.isEmpty(snapshotKey) && await this.database.ref(childPath).remove();
            }
      ...
      !result && await this.database.ref(path).remove();
    } catch (error) {
  ...
  send = async(action, onCallback, timeout = 8, retries)=>{
    ...
    if(!_.isEmpty(uid)){
      ...
      // await this.database.ref(chanel).remove().catch(console.log);
